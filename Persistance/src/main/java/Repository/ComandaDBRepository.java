...
//            }
    ...
    public void update(Comanda elem, Integer id) {
        try (
            ...
            session.beginTransaction();
            var query =
                    session.createSQLQuery("update Comenzi set statusComanda=?, farmacist=? where id=? ");
            query.setParameter(1, elem.getStatusComanda().ordinal());
            query.setParameter(2, elem.getFarmacist());
