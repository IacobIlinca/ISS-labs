...
public class SpitalClientRpcWorker implements Runnable, ISpitalObserver {
    ...
    public void updateMedicament() throws SpitalException {
        Response resp = new Response.Builder().type(ResponseType.ADDED_MEDICAMENT).build();
        System.out.println("Added result");
        try {
            sendResponse(resp);
        } catch (IOException e) {
    ...
    public void updateElementComanda() throws SpitalException {
        Response resp = new Response.Builder().type(ResponseType.UPDATED_ELEM_CMD).build();
...
//            int pid=(int)request.data();
        ...
        if (request.type() == RequestType.FIND_ALL_ELEM_CMD) {
            System.out.println("Get all elemente comanda");
            try {
                List<ElementComanda> medici = server.findAllElementeComanda(this);
                return new Response.Builder().type(ResponseType.FOUND_ALL_ELEM_CMD).data(medici).build();
            } catch (SpitalException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        ...
        if (request.type() == RequestType.ADD_MEDICAMENT) {
            Medicament cmd = (Medicament) request.data();
            System.out.println("Adding comanda");
            try {
                server.addMedicament(cmd, this);
                return new Response.Builder().type(ResponseType.OK).build();
            } catch (SpitalException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        if (request.type() == RequestType.STERGE_COMANDA) {
            Comanda cmd = (Comanda) request.data();
            System.out.println("Deleting comanda");
            try {
                server.stergeComanda(cmd, this);
                return new Response.Builder().type(ResponseType.OK).build();
            } catch (SpitalException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        if (request.type() == RequestType.STERGE_MEDICAMENT) {
            Medicament cmd = (Medicament) request.data();
            System.out.println("Deleting medicament");
            try {
                server.stergeMedicament(cmd, this);
                return new Response.Builder().type(ResponseType.OK).build();
            } catch (SpitalException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        if (request.type() == RequestType.UPDATE_MEDS) {
            Medicament cmd = (Medicament) request.data();
            System.out.println("Updating medicament");
            try {
                server.updateMedicament(cmd, this);
                return new Response.Builder().type(ResponseType.OK).build();
            } catch (SpitalException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        if (request.type() == RequestType.UPDATE_ELEM_CMD) {
            ElementComanda cmd = (ElementComanda) request.data();
            System.out.println("Updating elemente comanda");
            try {
                server.updateElementComanda(cmd, this);
                return new Response.Builder().type(ResponseType.OK).build();
            } catch (SpitalException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        if (request.type() == RequestType.UPDATE_COMANDA) {
            Comanda cmd = (Comanda) request.data();
            System.out.println("Updating comanda");
            try {
                server.updateComanda(cmd, this);
                return new Response.Builder().type(ResponseType.OK).build();
            } catch (SpitalException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
