...
public class SpitalServicesRpcProxy implements ISpitalService {
    ...
    @Override
    public List<ElementComanda> findAllElementeComanda(ISpitalObserver client) throws SpitalException {
        Request req = new Request.Builder().type(RequestType.FIND_ALL_ELEM_CMD).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
            String err = response.data().toString();
        ...
        List<ElementComanda> medici = (List<ElementComanda>) response.data();
        return medici;
    }
    ...
    @Override
    public void addMedicament(Medicament med, ISpitalObserver client) throws SpitalException {
        Request req = new Request.Builder().type(RequestType.ADD_MEDICAMENT).data(med).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
            String err = response.data().toString();
    ...
    @Override
    public void stergeComanda(Comanda comanda, ISpitalObserver client) throws SpitalException {
        Request req = new Request.Builder().type(RequestType.STERGE_COMANDA).data(comanda).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
            String err = response.data().toString();
    ...
    @Override
    public void stergeMedicament(Medicament medicament, ISpitalObserver client) throws SpitalException {
        Request req = new Request.Builder().type(RequestType.STERGE_MEDICAMENT).data(medicament).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
            String err = response.data().toString();
    ...
    @Override
    public void updateMedicament(Medicament medicament, ISpitalObserver client) throws SpitalException {
        Request req = new Request.Builder().type(RequestType.UPDATE_MEDS).data(medicament).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
            String err = response.data().toString();
    ...
    @Override
    public void updateElementComanda(ElementComanda elem, ISpitalObserver client) throws SpitalException {
        Request req = new Request.Builder().type(RequestType.UPDATE_ELEM_CMD).data(elem).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
            String err = response.data().toString();
    ...
    @Override
    public void updateComanda(Comanda cmd, ISpitalObserver client) throws SpitalException {
        Request req = new Request.Builder().type(RequestType.UPDATE_COMANDA).data(cmd).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
            String err = response.data().toString();
    ...
    }private void handleUpdateElementeComanda(Response response) {

        System.out.println("Proxy added result elemente comanda");
        try {
            client.updateElementComanda();
        } catch (SpitalException e) {
            e.printStackTrace();
        }
    ...
    private boolean isUpdateMedicamente(Response response) {
        return response.type() == ResponseType.ADDED_MEDICAMENT;
    }private boolean isUpdateElementeComanda(Response response) {
        return response.type() == ResponseType.UPDATED_ELEM_CMD;
    }
        ...
        public void run() {
            while (!finished) {
                try {
                    ...
                    }else if(isUpdateElementeComanda((Response) response)){
                        handleUpdateElementeComanda((Response) response);
                    }
                    else {
