...
public class SpitalServicesImpl implements ISpitalService {
    ...
    public List<ElementComanda> findAllElementeComanda(ISpitalObserver client) throws SpitalException {
        return repoElementComanda.findAll();
    }
    ...
    public void addMedicament(Medicament med, ISpitalObserver client) throws SpitalException {
        repoMedicament.add(med);
        ExecutorService executor = Executors.newFixedThreadPool(5);
        for (Integer key : loggedClients.keySet()) {
            ISpitalObserver receiver = loggedClients.get(key);
            executor.execute(() -> {
                try {
                    receiver.updateMedicament();
                } catch (SpitalException e) {
                    System.out.println("Error notifying referee.");
                }
            });
    ...
    public void stergeComanda(Comanda comanda, ISpitalObserver client) throws SpitalException {
        List<ElementComanda> elemente = comanda.getElementeComanda();
        for (var elem : elemente) {
            repoElementComanda.delete(elem);
        }
        repoComanda.delete(comanda);
        ExecutorService executor = Executors.newFixedThreadPool(5);
        for (Integer key : loggedClients.keySet()) {
            ISpitalObserver receiver = loggedClients.get(key);
            executor.execute(() -> {
    ...
    public void stergeMedicament(Medicament medicament, ISpitalObserver client) throws SpitalException {
        repoMedicament.delete(medicament);
        ExecutorService executor = Executors.newFixedThreadPool(5);
        for (Integer key : loggedClients.keySet()) {
            ISpitalObserver receiver = loggedClients.get(key);
            executor.execute(() -> {
    ...
    public void updateMedicament(Medicament medicament, ISpitalObserver client) throws SpitalException {
        repoMedicament.update(medicament, medicament.getId());
        ExecutorService executor = Executors.newFixedThreadPool(5);
        for (Integer key : loggedClients.keySet()) {
            ISpitalObserver receiver = loggedClients.get(key);
            executor.execute(() -> {
    ...
    public void updateElementComanda(ElementComanda elem, ISpitalObserver client) throws SpitalException {
        repoElementComanda.update(elem, elem.getId());
        ExecutorService executor = Executors.newFixedThreadPool(5);
//        loggedClients.(client);
        for (Integer key : loggedClients.keySet()) {
            ISpitalObserver receiver = loggedClients.get(key);
            executor.execute(() -> {
                try {
                    receiver.updateElementComanda();
                } catch (SpitalException e) {
                    System.out.println("Error notifying referee.");
                }
            });
    ...
    public void updateComanda(Comanda cmd, ISpitalObserver client) throws SpitalException {
        repoComanda.update(cmd, cmd.getId());
        ExecutorService executor = Executors.newFixedThreadPool(5);
//        loggedClients.(client);
        for (Integer key : loggedClients.keySet()) {
            ISpitalObserver receiver = loggedClients.get(key);
